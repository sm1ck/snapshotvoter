...
export const dispatchHelpEvent = (type, message) => {
  const helpEvent = new CustomEvent(EVENTS_TYPE.HELP.toString(), {
    detail: {
      type,
      message,
    },
...
export const dispatchCheckBoxEvent = (type) => {
  const checkBoxEvent = new CustomEvent(EVENTS_TYPE.CHECKBOX.toString(), {
    detail: {
      type,
    },
  });
...
export const validateProposal = (s) =>
  String(s).toLowerCase().startsWith("0x") &&
  String(s).toLowerCase().length === 66;

// Проверка на число с дефисом

export const validateIntegerWithHyphen = (s) =>
  isInteger(s) || String(s).includes("-")
    ? String(s)
        .split("-")
        .every((e) => isInteger(e))
    : false;
  ...
  switch (type) {
    ...
    case INPUT_TYPE.PROPOLSAL:
      return {
        label: "ID проползала:",
        placeholder:
          "например 0x4218ee725798f5450583c29e20fa231c782b9f6880ef9170229235e9eb0442ef",
