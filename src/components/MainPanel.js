...
export default function MainPanel() {
  ...
  useEffect(() => {
    if (lastJsonMessage !== null) {
      ...
      if (
        lastJsonMessage.type === WS_TYPE.END ||
        lastJsonMessage.type === WS_TYPE.ERROR_END
      ) {
        setStarted(false);
      }
    }
  ...
  return (
    ...
    >
      {page === 0 ? (
        <Title
          title={`${process.env.REACT_APP_NAME} v${process.env.REACT_APP_VERSION} | Главная`}
        />
      ) : (
        <Title
          title={`${process.env.REACT_APP_NAME} v${process.env.REACT_APP_VERSION} | Логи`}
        />
      )}
      <Toast />
      <HeadWrapper>
